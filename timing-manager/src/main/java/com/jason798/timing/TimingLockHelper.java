...
public class TimingLockHelper {
    private static Logger LOG = LoggerFactory.getLogger(TimingLockHelper.class);
    @Resource
    ...
    public boolean lock(Long tid) {
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("get tid {},mutex {},mutex tm {},interval {},alive {},aliveInterval {}",
                        tid, mutex, mutexTm, mutexInterval, aliveTm, aliveInterval);
            }

            if ((StringCheckUtil.equal(mutex, CommonConstant.Y)
                    && mutexInterval < TimingConstant.MUTEX_INTERVAL
            ) || (
                    TimingConstant.validCanExeStatus(status) && aliveInterval < TimingConstant.NOT_ALIVE_INTERVAL_PLUS)
                    ) {
